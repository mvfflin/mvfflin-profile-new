...
export default function Navbar() {
  ...
  return (
    <>
      <nav
        className={`hidden md:flex fixed top-0 left-0 h-full w-20 md:w-24 z-50 flex-col items-center py-8 transition-all duration-500 ${sidebarBg} backdrop-blur-xl border-r ${isDark ? "border-white/5" : "border-black/5"}`}
      >
          ...
          {navLinks.map((link) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`text-[9px] md:text-[10px] font-semibold mt-1.5 transition-colors duration-300 ${isActive ? "text-accent" : "text-muted"} group-hover:text-accent`}
                >
                  {link.name}
                </span>
              </a>
        ...
        <div className="mt-auto flex flex-col items-center gap-3">
          ...
          >
            <svg
              className={`w-5 h-5 transition-all duration-500 ${isDark ? "rotate-0 text-amber-400" : "rotate-0 text-amber-500"} group-hover:rotate-180 group-hover:text-accent`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
      ...
      <div
        className={`fixed top-0 left-0 h-16 w-full z-40 flex items-center justify-between px-6 md:px-10 transition-all duration-300 ${scrolled ? (isDark ? "bg-black/40" : "bg-white/40") : "bg-transparent"} backdrop-blur-md md:hidden`}
      >
        ...
        <button
          ...
          className={`p-2 rounded-lg ${isDark ? "hover:bg-white/5" : "hover:bg-black/5"} transition-colors`}
        >
          {isOpen ? (
            <svg
              className="w-6 h-6"
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          />
          <div
            className={`absolute right-0 top-0 h-full w-64 sm:w-72 ${isDark ? "bg-black/95" : "bg-white/95"} backdrop-blur-xl border-l ${isDark ? "border-white/5" : "border-black/5"} flex flex-col items-center py-8`}
          >
              ...
              {navLinks.map((link) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`text-xs font-semibold mt-1 transition-colors duration-300 ${isActive ? "text-accent" : "text-muted"} group-hover:text-accent`}
                    >
                      {link.name}
                    </span>
                  </a>
